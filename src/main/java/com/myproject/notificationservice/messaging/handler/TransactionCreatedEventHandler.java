...
public class TransactionCreatedEventHandler {
    ...
    @KafkaHandler
    public void handleEvent(@Payload TransactionCreatedEvent transactionCreatedEvent, @Header("messageId") String messageId) {


        if(transactionNotificationService.messageExists(messageId)){
            log.info("Message with ID {} already processed. Skipping processing.", messageId);
            return;
        }
